...
1. **File IO**
    - To make more than one processing request
2. **Functional Programming**
   - Due to the figures that we want to draw, functional programming makes sense as a solution to the problem
3. **Recursion**
    - Because of the recursive nature of fractal figures and other types of figures, recursion is the way to go.
...
- Racket
- 2htdp package (not in the standard Racket libraries)
...
Clone or download the repository. Read the [Instructions.txt](https://github.com/cxrlos/Numerical-Methods-Solver/blob/master/Instructions.txt) file to modify the [Configurations.txt](https://github.com/cxrlos/Numerical-Methods-Solver/blob/master/Configurations.txt) file.

Run the program by entering Racket:
...
~~~sh
(enter! "Main.rkt")
(generate_fractals "Configurations.txt")
~~~